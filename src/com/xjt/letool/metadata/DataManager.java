...
import com.xjt.letool.metadata.source.LocalSource;
...
public class DataManager {
    ...
    public static final int INCLUDE_LOCAL_IMAGE_SET_ONLY = INCLUDE_LOCAL_SET_ONLY | INCLUDE_IMAGE_SET;
    public static final int INCLUDE_LOCAL_VIDEO_SET_ONLY = INCLUDE_LOCAL_SET_ONLY | INCLUDE_VIDEO_SET;
    public static final int INCLUDE_LOCAL_ALL_SET_ONLY = INCLUDE_LOCAL_SET_ONLY | INCLUDE_IMAGE_SET | INCLUDE_VIDEO_SET;
    ...
    private void addSource(MediaSource source) {
        LLog.i(TAG, "addSource:" + source.getPrefix());
    ...
    public MediaObject getMediaObject(MediaPath path) {
        MediaObject obj = path.getObject();
        if (obj != null)
            return obj;
        LLog.i(TAG, "getMediaObject:" + path.getPrefix() + ":" + path.getPrefix().split("/")[1]);
        MediaSource source = mSourceMap.get(path.getPrefix().split("/")[1]);
        if (source == null) {
            LLog.w(TAG, "cannot find media source for path: " + path);
            return null;
        }
        ...
        try {
            ...
            if (object == null) {
                LLog.w(TAG, "cannot create media object: " + path);
            }
            return object;
        } catch (Throwable t) {
            LLog.w(TAG, "exception in creating media object: " + path, t);
            return null;
        }
    }

    public MediaObject getMediaObject(String prefix, int identity) {
        return getMediaObject(new MediaPath(prefix, identity));
    }

    public MediaSet getMediaSet(MediaPath path) {
    ...
    public void broadcastLocalDeletion() {
        LocalBroadcastManager manager = LocalBroadcastManager.getInstance(mApplication.getAppContext());

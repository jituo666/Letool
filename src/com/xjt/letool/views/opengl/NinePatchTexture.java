...
public class NinePatchTexture extends ResourceTexture {
    ...
    private NinePatchInstance findInstance(GLESCanvas canvas, int w, int h) {
        int key = w;
        key = (key << 16) | h;
...
class NinePatchInstance {
    ...
    private void prepareBuffers(GLESCanvas canvas) {
    ...
    public void draw(GLESCanvas canvas, NinePatchTexture tex, int x, int y) {
        if (mXyBufferName == -1) {
    ...
    public void recycle(GLESCanvas canvas) {
        if (mXyBuffer == null) {

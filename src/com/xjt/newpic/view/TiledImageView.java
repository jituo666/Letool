...
import com.xjt.newpic.common.ThreadPool.CancelListener;
...
public class TiledImageView extends GLView {
    ...
    public TiledImageView(LetoolContext context) {
        ...
        if (sTileSize == 0) {
            if (isHighResolution(context.getActivityContext())) {
                sTileSize = 512 ;
            } else {
                sTileSize = 256;
            }
    ...
    protected void render(GLESCanvas canvas) {
        ...
        int flags = 0;
        if (rotation != 0) flags |= GLESCanvas.SAVE_FLAG_MATRIX;
    ...
    private void uploadBackgroundTiles(GLESCanvas canvas) {
    ...
    void queueForUpload(Tile tile) {
        ...
        if (mTileUploader.mActive.compareAndSet(false, true)) {
            getGLController().addOnGLIdleListener(mTileUploader);
        }
    }
        ...
        synchronized (this) {
            if (tile.mTileState == STATE_RECYCLING) {
                ...
                if (tile.mDecodedTile != null) {
                    LetoolBitmapPool.getInstance().put(tile.mDecodedTile);
    ...
    private class TileUploader implements GLController.OnGLIdleListener {
    ...
    public void drawTile(GLESCanvas canvas,
    ...
    static boolean drawTile(Tile tile, GLESCanvas canvas, RectF source, RectF target) {
        while (true) {
            if (tile.isContentValid()) {
    ...
    private class Tile extends UploadedBitmapTexture {
        ...
        protected void onFreeBitmap(Bitmap bitmap) {
            LetoolBitmapPool.getInstance().put(bitmap);
        }
        ...
        boolean decode() {
            ...
            try {
                mDecodedTile = BitmapDecodeUtils.ensureGLCompatibleBitmap(mModel.getTile(
                        mTileLevel, mX, mY, sTileSize));
            } catch (Throwable t) {
                LLog.w(TAG, "fail to decode tile", t);
            }
